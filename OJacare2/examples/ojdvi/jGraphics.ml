...
and jGrView =
  object
    ...
    method makeImage : int32 OjArray.t -> int -> int -> jImage
  end
and jCamlEvent =
  object
...
and _capsule_jGrView (jni_ref : _jni_jGrView) =
  ...
  in
    object (self)
      method makeImage =
        fun (_p0 : int32 OjArray.t) _p1 _p2 ->
          let _p2 = Int32.of_int _p2 in
          let _p1 = Int32.of_int _p1 in
          let _p0 = OjArray.unwrap_int_array _p0
          in
            (new _capsule_jImage
               (Java.call
...
and _capsule_jCamlEvent (jni_ref : _jni_jCamlEvent) =
  ...
  in
    object (self)
      method get_key =
        fun () -> Java.get "mypack.CamlEvent.key:char" jni_ref
...
and virtual _souche_ml_dvi = let jni_ref = ref Java.null
  in
    object (self)
      initializer
        (jni_ref :=
           Java.proxy "mypack.MlDvi"
             (object
                method run =
                  fun _p0 (_p1 : _jni_jGrView) (_p2 : _jni_jGrControler) ->
                    let _p2 : jGrControler = new _capsule_jGrControler _p2 in
                    let _p1 : jGrView = new _capsule_jGrView _p1 in
                    let _p0 = JavaString.to_string _p0
                    in self#run _p0 _p1 _p2
              end);
         if Java.is_null !jni_ref
         then raise (Null_object "callback.mypack.CB_MlDvi")
         else ())
      method virtual run : string -> jGrView -> jGrControler -> unit
      method _get_jni_ml_dvi = (!jni_ref :> _jni_ml_dvi)
    end;;
...
class virtual _stub_ml_dvi () = object (self) inherit _souche_ml_dvi end;;
let mypack_jDviFrame__main (_p0 : ml_dvi) =
  let _p0 = _p0#_get_jni_ml_dvi
  in Java.call "mypack.DviFrame.main(mypack.MlDvi):void" _p0;;
let mypack_jGrView__get_transp = fun () ->
   Int32.to_int ( Java.get "mypack.GrView.transp" ());;
let mypack_jCamlEvent__get_MOUSE_MOTION_MASK = fun () ->
   Int32.to_int ( Java.get "mypack.CamlEvent.MOUSE_MOTION_MASK" ());;
let mypack_jCamlEvent__get_BUTTON_UP_MASK =fun () ->
   Int32.to_int ( Java.get "mypack.CamlEvent.BUTTON_UP_MASK"());;
let mypack_jCamlEvent__get_BUTTON_DOWN_MASK =fun () ->
   Int32.to_int ( Java.get "mypack.CamlEvent.BUTTON_DOWN_MASK"());;
let mypack_jCamlEvent__get_KEY_PRESSED_MASK =fun () ->
   Int32.to_int ( Java.get "mypack.CamlEvent.KEY_PRESSED_MASK"());;
