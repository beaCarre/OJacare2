...
Les tableaux Java dans \emph{OCaml-Java} correspondent à une instance d'un type donné dans le tableau suivant (en colonne 2).

Mais dans le but de faciliter leur manipulation, nous avons choisi d'encapsuler les tableaux, respectant ainsi le système de types d'O'Jacaré 2.
Les tableaux Java peuvent être encapsulés dans des types abstraits, simplifiant leur usage grâce à un accesseur, un modifieur, une fonction rendant leur taille et un fonction permettant leur création (initialisé avec une valeur par défaut) :
\newline
...
Pour plus d'informations, le module \emph{OjArray} est détaillé en annexe dans la section \ref{5.5}.

\subsection{Gestion du callback}
O'Jacaré permet de sous-classer des classes Java en OCaml et d'implémenter des interfaces Java en OCaml. 
L'utilisateur devra ajouter un attribut \emph{callback} devant la définition de l'interface dans l'IDL, devant celle de la classe, ou devant la ou les méthodes voulant être redéfinies.
Cela nécessite un mécanisme permettant à du code Java de revenir au monde OCaml :
...
Elle contient une référence sur une instance Java créée grâce à l'outil \emph{Java.proxy}, 

La nouvelle classe OCaml doit hériter d'une classe (souche) portant une référence sur cette classe fille, et l'utilisateur peut ainsi redéfinir des méthodes de la classe Java.
...
\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.7]{schemaCBClass.pdf}
\end{figure}
...
\bibitem{O'Jacare} HENRY G., \emph{O’Jacaré} \url{http://www.pps.univ-paris-diderot.fr/~henry/ojacare/}

\bibitem{OCaml-Java} CLERC X.,\emph{OCaml-Java 2.0} \url{http://ocamljava.x9c.fr/preview/}

\bibitem{O'Jacare2} CHAILLOUX E., HENRY G., \emph{O’Jacaré, une interface objet
